...
export class ParkingService {
    ...
    getParkingData(limit: number = 20, page: number = 1): Promise<Parking[]> {
        const API_URL = this.apiUrlService.apiParkingLocationData();
        return firstValueFrom(this.http.get<ParkingResponse>(`${API_URL}?limit=${limit}&page=${page}`))
            .then(res => res.data as Parking[]);
    }
