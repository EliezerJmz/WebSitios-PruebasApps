...
export class UserService {
    ...
    getUserData(limit: number = 20, page: number = 1): Promise<User[]> {
        return firstValueFrom(this.http.get<UserResponse>(`${this.API_URL}?limit=${limit}&page=${page}`))
            .then(res => res.data as User[]);
    }

    getRolData(limit: number = 20, page: number = 1): Promise<Role[]> {
        return firstValueFrom(this.http.get<RoleResponse>(`${this.ROLE_API_URL}?limit=${limit}&page=${page}`))
