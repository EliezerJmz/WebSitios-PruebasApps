...
export class UsersComponent implements OnInit {
    ...
    getUserData() {
        this.userService.getUserData()
            ...
            .catch(error => {
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Error al traer la información de la Base de datos, revisa tu conexión.' });
            });
    }
    ...
    loadRoles(): void {
        this.userService.getRolData()
            .then(data => {
                this.roles = data.map(role => ({ label: role.name, value: role.id }));
            })
            .catch(error => {
            ...
            this.notificationsService.getUserPreferences(user.id).subscribe(
                ...
                (error) => {
                    this.messageService.add({
                        severity: 'error',
                        summary: 'Error',
                        detail: 'No se pudieron cargar las preferencias del usuario',
                        life: 3000
                    });
                }
            );
