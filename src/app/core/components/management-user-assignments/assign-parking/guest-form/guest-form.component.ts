...
export class GuestFormComponent implements OnInit{
    ...
    ngOnInit(): void {
        ...
        this.settingService.getSetting().then((data) => {
          ...
          //console.log(maxDaysToAssignmentLoan); //(como número)
...
loadAssignments(){
  ...
  removeAssignment(assignmentLoanId: string){
    console.log("LOAN ID", assignmentLoanId)
    this.assignmentLoanService.deleteAssignmentLoan(assignmentLoanId).subscribe({
        next: (response: any)=> {
            console.log('*** Asignación Invitado Eliminado ****:', response)
            this.messajeCreated("danger","Asignación Temporal Eliminad");
            setTimeout( () => { this.redirectSearchAssignment(); }, 2000 );
